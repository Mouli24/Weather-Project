...
def validate_city_name(city: str) -> bool:
    """
    Valid city names contain letters and optional spaces/hyphens.
    Reject numeric or special-character-only inputs.
    """
    pattern = r"^[A-Za-z\s\-]+$"
...
def build_city_list(args, cfg) -> list[str]:
    ...
    # args.city DOES NOT EXIST — so removed

    # args.cities is a list because of action="append"
    if args.cities:
        cities.extend(args.cities)

    # If nothing provided, use config.ini default
    if not cities:
        cities = [cfg["city"]]
    ...
    for c in cities:
        if validate_city_name(c):
            ...
            print(f"\n❌ Invalid city name: {c}")
            print("City names must contain only letters, spaces, or hyphens.")
